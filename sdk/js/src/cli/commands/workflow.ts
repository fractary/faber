...
import { FaberWorkflow } from '../../workflow';
...
workflowCommand
  ...
  .action((workflowId: string) => {
    try {
      ...
      console.log('Work ID:', (status.state as unknown as { work_id: string }).work_id);
      console.log('Status:', (status.state as unknown as { status: string }).status);
