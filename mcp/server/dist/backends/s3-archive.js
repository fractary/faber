...
export class S3ArchiveBackend {
    config;
    localBasePath;
    constructor(config, localBasePath) {
        this.config = config;
    ...
    async archiveRun(runId) {
        ...
        try {
            ...
            if (this.config.cleanupLocalAfterArchive &&
    ...
    async restoreRun(runId) {
        ...
        try {
            ...
            filesRestored.push(...files.map((f) => f.replace(`${localRunDir}/`, "")));
            ...
            if (fs.existsSync(jsonlPath) &&
                ...
                for (const line of lines) {
                    try {
                        ...
                        fs.writeFileSync(path.join(eventsDir, filename), JSON.stringify(event, null, 2));
...
//# sourceMappingURL=s3-archive.js.map