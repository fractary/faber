 ...
 */
export function createEventTools(backend) {
    return [
        // Tool 1: fractary_faber_event_emit (migrated from emit_event)
        {
            ...
            handler: async (params) => {
                const result = await backend.emitEvent(params);
                return {
                    content: [
                        {
                            type: 'text',
        ...
        // Tool 2: fractary_faber_run_get (migrated from get_run)
        {
            ...
            handler: async (params) => {
                const { run_id, include_events } = params;
        ...
        // Tool 3: fractary_faber_run_list (migrated from list_runs)
        {
            ...
            handler: async (params) => {
                const result = await backend.listRuns(params);
                return {
                    content: [
                        {
                            type: 'text',
