 ...
 */
function createToolError(toolName: string, error: string) {
  return {
    content: [{
      type: 'text',
  ...
  return [
    // Tool 1: fractary_faber_workflow_run
    {
      ...
      handler: async (params) => {
        const { work_id, autonomy, config } = params as {
          work_id: string;
          autonomy?: string;
          config?: unknown;
        };

        const workflow = new FaberWorkflow({ config: config as never });
        try {
          const result = await workflow.run({
            workId: work_id,
            autonomy: (autonomy || 'assisted') as 'dry-run' | 'assisted' | 'guarded' | 'autonomous',
          ...
          return createToolError('workflow_run', (error as Error).message);
        }
      },
    },
    ...
    // Tool 2: fractary_faber_workflow_status
    {
      ...
      handler: async (params) => {
        const { workflow_id, work_id } = params as {
          workflow_id?: string;
          work_id?: string;
        };
    ...
    // Tool 3: fractary_faber_workflow_resume
    {
      ...
      handler: async (params) => {
        const { workflow_id } = params as { workflow_id: string };
        ...
        try {
          ...
          return createToolError('workflow_resume', (error as Error).message);
        }
      },
    },
    ...
    // Tool 4: fractary_faber_workflow_pause
    {
      ...
      handler: async (params) => {
        ...
        try {
          ...
          return createToolError('workflow_pause', (error as Error).message);
        }
      },
    },
    ...
    // Tool 5: fractary_faber_workflow_recover (Option B: Direct StateManager)
    {
      ...
      handler: async (params) => {
        const { workflow_id, checkpoint_id, from_phase, skip_phases } = params as {
          workflow_id: string;
        ...
        try {
          ...
          return createToolError('workflow_recover', (error as Error).message);
        }
      },
    },
    ...
    // Tool 6: fractary_faber_workflow_cleanup (Option B: Direct StateManager)
    {
      ...
      handler: async (params) => {
        ...
        try {
          ...
          return createToolError('workflow_cleanup', (error as Error).message);
        }
      },
    },
  ];
