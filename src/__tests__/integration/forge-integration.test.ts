...
describe('FABER + Forge Integration', () => {
  ...
  describe('Agent Resolution', () => {
    ...
    it('should handle agent not found gracefully', async () => {
      ...
      await expect(
        executor.executePhaseAgent('frame', 'Task', context)
      ).rejects.toThrow(/Agent 'frame-agent' not found/);
    });
  });
  ...
  describe('Dual-Mode Support', () => {
    it('should support switching between Forge and legacy modes', async () => {
      ...
      const forgeWorkflow = new FaberWorkflow({
        config: {
          forge: { enabled: true, prefer_local: true },
        },
      });

      expect(forgeWorkflow).toBeInstanceOf(FaberWorkflow);

      // Create with legacy mode
      const legacyWorkflow = new FaberWorkflow({
        config: {
          forge: { enabled: false, prefer_local: true },
        },
      });

      expect(legacyWorkflow).toBeInstanceOf(FaberWorkflow);
    });
