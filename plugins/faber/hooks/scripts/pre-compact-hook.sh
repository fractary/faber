...
jq -n \
  ...
  '{
    hookSpecificOutput: {
      ...
      additionalContext: ("Context compaction imminent. Finish your current step if mid-step, update state.json to mark it completed, then output exactly: CONTEXT_YIELD: run_id=" + $run_id + " resume_cmd=/fractary-faber-workflow-run " + $work_id + " --resume " + $run_id + " — then stop. Do NOT mark subsequent steps complete.")
    }
  }'
