...
{if auto_fix_applied and all_fixes_successful and not auto_learn_mode and not learn_mode:}
...
**Goal**: Add successful fix to knowledge base for future reference via Codex MemoryWriter
...
if learn_mode:
  ...
  # Try Codex MemoryWriter first, fall back to legacy create_kb_entry_markdown
  TRY:
    codex_adapter = get_codex_adapter()
    if codex_adapter.isAvailable():
      # Use MemoryWriter via codex-adapter
      result = codex_adapter.writeMemory({
        memory_type: "troubleshooting",
        title: sanitize_title(problems[0].description),
        description: "Resolved: {problems[0].description}. Root cause: {root_causes[0].root_cause}",
        body: generate_memory_body(problems[0], root_causes[0], solutions[0]),
        frontmatter: {
          category: problems[0].category,
          severity: "medium",
          status: "verified",
          symptoms: [problems[0].description],
          agents: [agent_parameter or "unknown"],
          phases: [problems[0].phase or phase_parameter or "unknown"],
          tags: ["auto-generated", verification_result or "manual-verified"],
          verified: true,
          success_count: 1
        }
      })

      PRINT "✅ Resolution saved to Codex memory"
      PRINT "   Memory ID: {result.memory_id}"
      PRINT "   File: {result.file_path}"
      if result.deduplicated:
        PRINT "   (Merged with existing memory {result.existing_id})"
    else:
      # Fall back to legacy KB creation
      ...
      PRINT "✅ Resolution added to knowledge base (legacy path)"
  CATCH codex_error:
    # Fall back to legacy KB creation
    WARN "Codex MemoryWriter unavailable, using legacy KB path: {codex_error}"
    create_kb_entry_markdown(
      problem=problems[0].description,
      category=problems[0].category,
      phase=problems[0].phase,
...
**Helper Function**: `generate_memory_body(problem, root_cause, solution)`
```
body = """
# {sanitize_title(problem.description)}
...
**Helper Function (Legacy Fallback)**: `create_kb_entry_markdown(problem, category, phase, step, agent, root_cause, solution, verification_result)`
```
...
# Save to legacy knowledge base path
...
- **Commands**:
  - `commands/fractary-faber-workflow-run.md` - Resume workflows after fixes
  - `commands/fractary-faber-run-inspect.md` - Check workflow run status
- **Knowledge Base**:
