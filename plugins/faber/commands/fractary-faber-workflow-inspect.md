...
allowed-tools: Agent(fractary-faber-workflow-inspector)
model: claude-haiku-4-5
argument-hint: '[<workflow-name-or-path>] [--verbose] [--fix] [--check <aspect>] [--config-path <path>]'
---

Use **Agent** tool with `fractary-faber-workflow-inspector` agent to validate workflow configuration with provided arguments.
