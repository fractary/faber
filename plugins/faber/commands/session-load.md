...
allowed-tools: Task(fractary-faber:session-loader)
model: claude-haiku-4-5
argument-hint: '[--work-id <id>] [--run-id <id>] [--trigger <trigger>] [--artifacts <list>] [--context <hint>] [--force] [--dry-run]'
---

Use **Task** tool with `fractary-faber:session-loader` agent to reload critical workflow artifacts with provided arguments.
